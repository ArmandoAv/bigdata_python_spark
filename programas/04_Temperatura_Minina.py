"""
    Obtener las temperaturas minimas de cada estacion de clima
...
"""
conf = SparkConf().setMaster("local").setAppName("TemperaturasMinimas")
...
"""
temp_estacion = temp_min.map(lambda x: (x[0], x[2]))

"""
    Se busca la temperatura minima por cada estacion
    reduceByKey agrega cada temperatura minima de cada estacion
    la funcion lambda determinara como hacemos esta accion
    en x, y: se combinan las temperaturas de la misma estacion
    con la funcion min() se encuentra el valor minimo de cada estacion
"""
temp_min_unica = temp_estacion.reduceByKey(lambda x, y: min(x, y))
...
print("Las temperaturas minimas por cada estacion son las siguientes: ")
